...
// Replace the entire welcome useEffect with this:
useEffect(() => {
  ...
  const handleSmartGameCoach = () => {
    ...
    const decision = GameCoachSessionManager.shouldShowMessage(
      ...
      sceneProgress,
      {
        ...
        quickNavigation: false // Will be set by click handlers
      }
    );
    
    console.log('🎭 GameCoach Decision:', decision);
    
    if (decision.shouldShow) {
  // ✅ ADD: Get profile name
  ...
  const message = GameCoachSessionManager.getMessage(
    ...
    decision.trigger, 
    decision,
    profileName  // ✅ PASS PROFILE NAME
  );
      
      // Show the message
      setTimeout(() => {
        // ✨ Different sparkle effects for different message types
    let sparkleType = 'divine-light'; // default
    
    switch(decision.messageType) {
      case 'welcome':
        sparkleType = 'divine-light-welcome';
        break;
      case 'celebration':
        sparkleType = 'divine-light-celebration';
        break;
      case 'encouragement':
        sparkleType = 'divine-light-encourage';
        break;
      case 'mastery':
        sparkleType = 'divine-light-mastery';
        break;
      default:
    ...
    setTimeout(() => {
      ...
      }, 1500); // Delay to allow zone to settle
    }
  };
  
  // Add slight delay to ensure scene progress is loaded
  const timer = setTimeout(handleSmartGameCoach, 500);
...
>
  🎭 DEBUG GC
      ...
      <div className="zone-scenes-container">
        {/* ✅ STEP 1: Replace your entire zoneData.scenes.map block with this: */}
      ...
      {zoneData.scenes.map((scene, index) => {
        ...
        return (
          ...
          >
            {/* 🔍 DEBUG: Add this button to see temp session data */}
<button 
  onClick={() => {
    const activeProfileId = localStorage.getItem('activeProfileId');
    const tempKey = `temp_session_${activeProfileId}_${zoneData.id}_${scene.id}`;
    ...
    console.log(`🔍 RAW TEMP DATA for ${scene.id}:`);
    ...
    if (tempData) {
      ...
      console.log('completed flag:', parsed.completed);
      console.log('phase:', parsed.phase);
      console.log('stars:', parsed.stars);
      console.log('discoveredSymbols:', parsed.discoveredSymbols);
    }
  }}
  style={{
    position: 'fixed',
  ...
  {status.status === 'in-progress' ? (
    ...
    >
      ...
      {status.status === 'locked' && '🔒 Complete Previous'}
            ...
            {/* ✅ NEW: Split-button design for in-progress scenes */}
<div className="scene-action-indicator">
  {status.status === 'completed' && (
    <div className="action-label completed" onClick={() => handleSceneClick(scene, 'replay')}>
      <span className="action-emoji">🎮</span>
      <span className="action-text">Play Again</span>
    </div>
  )}
  
  {status.status === 'in-progress' && (
    <div className="action-label-split">
      <div 
      ...
      >
        <span className="action-emoji">↶</span>
        <span className="action-text">Continue ({status.stars}⭐)</span>
      </div>
      <div 
  ...
  {status.status === 'available' && (
    <div className="action-label available" onClick={() => handleSceneClick(scene, 'start')}>
      <span className="action-emoji">🚀</span>
      <span className="action-text">Start Adventure</span>
    </div>
  )}
  
  {status.status === 'locked' && (
    <div className="action-label locked">
      <span className="action-emoji">🔒</span>
      <span className="action-text">Complete Previous</span>
    </div>
  )}
</div>
          </div>
