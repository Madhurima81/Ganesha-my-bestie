...
const checkProgress = (profileId) => {
  ...
  console.log('🔍 PROGRESS CHECK: Starting for profile:', profileId);
   ...
   // ✅ ADD THIS BLOCK: Check for Play Again flag first
  /*const playAgainKeys = [
    `play_again_${profileId}_symbol-mountain_final-scene`,
    `play_again_${profileId}_symbol-mountain_sacred-assembly`
  ];
  
  const wasPlayAgain = localStorage.getItem(`was_play_again_${profileId}`);
  const hasPlayAgainFlag = playAgainKeys.some(key => localStorage.getItem(key) === 'true');
  
  if (wasPlayAgain === 'true' || hasPlayAgainFlag) {
    console.log('🎮 PLAY AGAIN DETECTED: Clearing progress flags and forcing fresh start');
    
    // Clear all play again flags
    playAgainKeys.forEach(key => localStorage.removeItem(key));
    localStorage.removeItem(`was_play_again_${profileId}`);

      
    // Clear temp sessions to force fresh start
    ...
    // Clear current scene to force restart
    SimpleSceneManager.clearCurrentScene();
    
    setHasProgress(false); // This will show "Start Adventure" instead of "Continue"
    return;
  }*/
...
const tempFinalKey = `temp_session_${ProfileId}_symbol-mountain_final-scene`;  // ← ADD THIS



if (localStorage.getItem(tempPondKey) || localStorage.getItem(tempModakKey) || localStorage.getItem(tempSymbolKey)) {  // ← ADD || localStorage.getItem(tempSymbolKey)
  ...
  if ((key.includes('sceneState') || 
       ...
       key.includes('modak') ||
       key.includes('pond') ||
      key.includes('symbol') ||  // ← ADD THIS LINE
           key.includes('final-scene') ||  // ← ADD THIS LINE
    ...
    (sceneState.placedSymbols && Object.keys(sceneState.placedSymbols).length > 0) ||  // ← ADD: Sacred Assembly progress
(sceneState.ganeshaState && sceneState.ganeshaState !== 'stone') ||  // ← ADD: Sacred Assembly progress
(sceneState.assemblyWisdomShown === true) ||  // ← ADD: Sacred Assembly progress
(sceneState.showingZoneCompletion === true)) {  // ← ADD: Sacred Assembly progress
...
const tempFinalKey = `temp_session_${activeProfileId}_symbol-mountain_final-scene`;  // ← ADD THIS
  
if (localStorage.getItem(tempPondKey) || localStorage.getItem(tempModakKey) || localStorage.getItem(tempSymbolKey) || localStorage.getItem(tempFinalKey)) {  // ← ADD || localStorage.getItem(tempSymbolKey)
