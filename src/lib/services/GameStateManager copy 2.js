...
class GameStateManager {
  ...
  gameProgress.zones[zoneId].scenes[sceneId] = {
    ...gameProgress.zones[zoneId].scenes[sceneId], // Preserve existing data like unlocked flag
    completed: sceneState.completed || false,
    stars: sceneState.stars || 0,
    symbols: sceneState.symbols || {},
    lastPlayed: Date.now(),
    completedAt: sceneState.completed ? Date.now() : (gameProgress.zones[zoneId].scenes[sceneId]?.completedAt || null)
  };
  
  console.log('🔄 Scene data updated:', gameProgress.zones[zoneId].scenes[sceneId]);
  
  // ✅ 4. CRITICAL FIX: Recalculate zone totals IMMEDIATELY
  let zoneStars = 0;
  let zoneCompletedScenes = 0;
  
  Object.values(gameProgress.zones[zoneId].scenes).forEach(scene => {
  ...
  gameProgress.zones[zoneId].stars = zoneStars;
  gameProgress.zones[zoneId].completed = zoneCompletedScenes === Object.keys(gameProgress.zones[zoneId].scenes).length;
  
  console.log(`📊 Zone ${zoneId} totals recalculated:`, {
    stars: zoneStars,
    completedScenes: zoneCompletedScenes,
    totalScenes: Object.keys(gameProgress.zones[zoneId].scenes).length
  });
  
  // ✅ 5. CRITICAL FIX: Recalculate overall totals
  let totalStars = 0;
  let totalCompletedScenes = 0;
  
  Object.values(gameProgress.zones).forEach(zone => {
  if (zone.scenes) {
    Object.values(zone.scenes).forEach(scene => {  // ← Make sure "scene" is here
  ...
  console.log('✅ GAME STATE SAVED COMPLETELY:', {
    zoneId,
    sceneId,
    zoneTotalStars: gameProgress.zones[zoneId].stars,
    zoneTotalCompleted: zoneCompletedScenes,
    profileTotalStars: gameProgress.totalStars,
    profileTotalCompleted: gameProgress.completedScenes
  });
