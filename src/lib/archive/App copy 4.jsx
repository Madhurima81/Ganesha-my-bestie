// App.jsx - Fixed with proper full-screen container
...
function App() {
  ...
  // Handle zone selection from map - 🌟 UPDATED: Support scene selection
  const handleZoneSelect = (zoneId, sceneId = null) => {
    console.log('Selecting zone:', zoneId, 'scene:', sceneId);
    setCurrentZone(zoneId);
    
    if (zoneId === 'symbol-mountain') {
      if (sceneId) {
        // Specific scene selected from map
        setCurrentScene(sceneId);
      } else {
        // No specific scene, determine based on progress
        try {
          const modakProgress = GameStateManager.getSceneProgress('symbol-mountain', 'modak');
          if (modakProgress && modakProgress.completed) {
            setCurrentScene('pond');   // Go to pond after modak is complete
          } else {
            setCurrentScene('modak');  // Start with modak first
          }
        } catch (error) {
          console.log('Error checking scene progress, defaulting to modak');
  ...
  return (
    <div className="App"> {/* 🔧 ADDED: Apply App CSS class */}
