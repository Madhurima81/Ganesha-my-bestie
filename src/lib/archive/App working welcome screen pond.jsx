// App.jsx - Updated with GameCoachProvider and fixed profile handling
...
function App() {
  ...
  const handleZoneSelect = (zoneId) => {
    ...
    // For now, if it's symbol-mountain, go directly to pond scene
    if (zoneId === 'symbol-mountain') {
      setCurrentScene('pond');
      setCurrentView('scene');
      
      // Save current location when entering a scene
      GameStateManager.saveGameState(zoneId, 'pond', {
        currentZone: zoneId,
        currentScene: 'pond',
        enteredAt: Date.now()
      });
  ...
  const handleSceneComplete = (sceneId, result) => {
    ...
    // Go back to map after scene completion
    setCurrentView('map');
  };
...
position: 'top-right'
      ...
      {currentView === 'scene' && (() => {
        ...
        if (currentZone === 'symbol-mountain' && currentScene === 'pond') {
          return (
            <>
              <PondScene 
