...
const handleSceneComplete = (sceneId, result) => {
  console.log('🎯 PROGRESS MANAGER: Scene completed:', sceneId, result);
  //restoreDefaultStyles(); // Restore styles after completion
  
  try {
    // ✅ Use ProgressManager for unified saving
    const activeProfileId = localStorage.getItem('activeProfileId');
  ...
  sceneId, 
  {
    ...
    symbols: result?.symbols || {}
      ...
      console.log('✅ PROGRESS MANAGER: Scene completion saved successfully');
      console.log('📊 PROGRESS MANAGER: Updated zone progress:', updatedZoneProgress);
      
      // ✅ Auto-unlock next scene (keep existing GameStateManager call)
      const unlockedScene = GameStateManager.unlockNextScene(currentZone, sceneId);
      if (unlockedScene) {
        console.log('🎉 PROGRESS MANAGER: Next scene auto-unlocked:', unlockedScene);
      } else {
        console.log('🏁 PROGRESS MANAGER: Last scene in zone completed');
      }
    } else {
      console.error('❌ PROGRESS MANAGER: Missing required data for scene completion', {
        activeProfileId,
        currentZone,
        sceneId,
        result
    ...
    console.log('🎯 PROGRESS MANAGER: Navigating to zone welcome to show updated progress');
    setCurrentScene(null);
    setCurrentView('zone-welcome');
    
  } catch (error) {
    console.error('❌ PROGRESS MANAGER: Error saving scene completion:', error);
    // Fallback navigation
    setCurrentScene(null);
    setCurrentView('zone-welcome');
  }
