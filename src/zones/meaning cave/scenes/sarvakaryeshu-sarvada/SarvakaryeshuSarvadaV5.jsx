...
const SarvakaryeshuSarvadaContent = ({
  ...
  const startScenario = (scenarioIndex = 0) => {
    ...
    sceneActions.updateState({
      ...
      ganeshaClicked: false,
        isCompletingScenario: false, // ← ADD THIS
      wiseChoicesMade: [],
      unwiseChoicesTried: [],
  ...
  const handleSymbolClick = (symbolId, isWise) => {
      // ✅ BLOCK clicks during scenario completion
  if (sceneState.isCompletingScenario) {
    console.log('🚫 Ignoring symbol click - scenario completing');
    return;
  }
  ...
  symbolsClickable: false, // ← ADD THIS
    symbolsVisible: 'none', // ← ADD THIS: Hide symbols immediately
  isCompletingScenario: true, // ← ADD THIS  
...
// ✅ IMMEDIATE: Show transformation text right away
setShowSparkle(null);
sceneActions.updateState({
  showGoldenHelperText: true
});

// ✅ AFTER 2 SECONDS: Hide text and start Game 2
setTimeout(() => {
  sceneActions.updateState({
...
const resetGame2HelperState = () => {
  sceneActions.updateState({
    ...
    isCompletingScenario: false, // ← ADD THIS
    symbolsClickable: true, // ← ADD THIS
    characterMessage: null,
    characterMessageType: null,
...
const handleGame2SymbolClick = (symbolId) => {
    // ✅ BLOCK clicks during scenario completion  
  if (sceneState.isCompletingScenario) {
    console.log('🚫 Ignoring helper click - scenario completing');
    return;
  }
  
    // ✅ FIX: Get scenario and helper data FIRST
  const scenario = getCurrentGame2Scenario();
  const helperData = scenario.helpers.find(h => h.id === symbolId);
    const helpMessage = getHelperMessage(symbolId, scenario); // ← ADD THIS LINE


  // ✅ ONLY handle FIRST CLICK now
  if (sceneState.selectedHelperSymbol !== symbolId) {
    console.log('🤔 FIRST CLICK - Showing helper on screen with speech bubble');
    ...
    sceneActions.updateState({
      ...
      sceneInstructionText: null, // ✅ Remove confusing text
      helperReadyToSend: true     // ✅ Enable send button
    });
  } else {
    console.log('🎯 Same helper clicked again - ignoring (use button instead)');
  }
};

// ✅ NEW: Function to handle sending the helper
const handleSendHelper = () => {
  console.log(`✨ Sending helper: ${sceneState.selectedHelperSymbol}`);
...
sceneActions.updateState({
  ...
  helperReadyToSend: false,
  symbolsClickable: false, // ← ADD THIS
    symbolsVisible: 'none', // ← ADD THIS: Hide symbols during animation
  isCompletingScenario: true, // ← ADD THIS
  characterMessage: "Helper sent! ✨",
  characterMessageType: 'sending'
});
  
// ❌ REMOVE THIS: Let animation handle completion
  // setTimeout(() => {
...
const completeGame2Scenario = () => {
  ...
  sceneActions.updateState({
    ...
    game2ScenariosCompleted: scenariosCompleted, // ← ADD THIS
    showHelperOnScreen: false,
    helperReadyToSend: false,
    selectedHelperSymbol: null,
    characterMessage: scenario.gratitude,
    characterMessageType: 'grateful' // ← ADD THIS
  });
  
  // ← ADD THIS: Ganesha blessing after 2 seconds
  setTimeout(() => {
    sceneActions.updateState({
      ganeshaEmerged: true,
      ganeshaBlessing: true,
  ...
  // ← ADD THIS: Next scenario or complete Game 2
  setTimeout(() => {
    if (scenariosCompleted >= 3) {
      completeGame2();
    } else {
 ...
 sceneState.phase === SCENE_PHASES.HELPER_ANIMATION && (
  <HelperSignatureAnimation
    ...
    toPosition={{ x: 50, y: 50 }}
        duration={3000} // ← ADD THIS: 3 second animation
...
.person-problem-speech {
  ...
  padding: 8px 12px; /* ← SMALLER: was 15px 20px */
  border-radius: 15px; /* ← SMALLER: was 20px */
  border: 2px solid #FF8C42;
  max-width: 280px; /* ← SMALLER: was 300px */
  font-size: 14px;
  color: #333;
  text-align: center;
            ...
            <button
              onClick={() => {
                ...
                sceneActions.updateState({
                  ...
                  showInAllTasksText: false,
                  showAlwaysText: false,
                  showGoldenHelperText: false, // ← ADD THIS LINE
