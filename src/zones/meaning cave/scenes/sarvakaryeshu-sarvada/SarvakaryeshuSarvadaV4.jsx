...
const SCENE_PHASES = {
  DOOR1_ACTIVE: 'door1_active',           // ← FIRST: Door 1
  DOOR1_COMPLETE: 'door1_complete',
  CHARACTER_SELECT: 'character_select',    // ← MOVED: After Door 1
  ...
  SCENARIO_SUCCESS: 'scenario_success',
    SARVAKARYESHU_LEARNING: 'sarvakaryeshu_learning', // ✅ NEW: Clean text phase
  DOOR2_ACTIVE: 'door2_active',           // ← After 3 scenarios
  DOOR2_COMPLETE: 'door2_complete',

  // ✅ ADD THESE GAME 2 PHASES HERE (after line 124):
  GAME2_TRANSFORMATION: 'game2_transformation',     // Golden helper transformation
  MISSION_UNCERTAINTY: 'mission_uncertainty',       // = scenario_uncertainty
  MISSION_CLARITY: 'mission_clarity',               // = scenario_clarity  
  MISSION_CHOOSING: 'mission_choosing',             // = scenario_choosing
  MISSION_SUCCESS: 'mission_success',               // = scenario_success
  GANESHA_GOLDEN_BLESSING: 'ganesha_golden_blessing', // Divine partnership moment
  GAME2_MISSION_COMPLETE: 'game2_mission_complete', // After helper animation
  SARVADA_LEARNING: 'sarvada_learning',             // "Always" text display

  TRANSFORMATION: 'transformation',
  GAME2_HELPER: 'game2_helper',
...
// ADD this after your SCENARIOS object (around line 400-500):

const HELPER_MISSIONS = {
  grandparents: {
    id: 'grandparents',
...
sceneClickable: false,       // Whether main scene can be clicked
          ...
          // Game 2 state (exact copy of Game 1 pattern)
game2CurrentMission: 0,                    // = currentScenario
game2SelectedMissions: [],                 // = selectedScenarios  
game2MissionPhase: 'uncertainty',         // = scenarioPhase
game2HelpersVisible: 'none',              // = symbolsVisible
game2HelpersClickable: false,             // = symbolsClickable
game2SelectedHelper: null,                // = selectedSymbol
game2SelectedHelperData: null,            // = selectedSymbolData
game2MissionClickable: false,             // = sceneClickable
game2ShowProblemSpeech: false,            // = showProblemDeclaration
game2ProblemSpeechShown: false,           // = problemDeclarationShown
game2CurrentBackground: 'before',         // = currentBackground
game2MissionsCompleted: 0,                // = scenariosCompleted
game2MissionHelpers: [],                  // Current mission's 3 helpers
game2CurrentMissionData: null,            // Current mission data
game2HelperPopupVisible: false,           // NEW: Center popup
game2HelperMessage: null,                 // NEW: Helper speech

// ✅ ADD these new state properties:
missionGratitudeVisible: false,
ganeshaPresent: false,
goldenChildActive: false,
divineMessage: null,
...
const SarvakaryeshuSarvadaContent = ({
  ...
  // ✅ NEW: Separate function for scenario window image
...
const handleDoor2Complete = () => {
  ...
  sceneActions.updateState({
    door2Completed: true,
    phase: SCENE_PHASES.GAME2_TRANSFORMATION,
    game2SelectedMissions: missions,
    // ✅ Set mission data immediately
    game2CurrentMissionData: firstMissionData,
    game2CurrentBackground: 'before',
    stars: 8
  });
  
  setTimeout(() => startMission(0), 2000); // Shorter wait
};
...
// ✅ NEW: Start Game 2 scenario (similar to Game 1 but different flow)
const startGame2Scenario = (scenarioIndex = 0) => {
  console.log(`🎯 Starting Game 2 scenario ${scenarioIndex + 1} - helping others`);
  ...
  sceneActions.updateState({
    ...
    helperSymbolsGenerated: randomSymbols,
    characterMessage: null,
    characterMessageType: null
  });
...
// ✅ ADD ALL GAME 2 FUNCTIONS HERE (after Door 2, before utilities):

// 🎯 Game 2: Generate random missions (copy generateRandomScenarios pattern)
const generateRandomMissions = () => {
  const missionKeys = Object.keys(HELPER_MISSIONS);
  const shuffled = [...missionKeys].sort(() => 0.5 - Math.random());
  return shuffled.slice(0, 3); // Take first 3 for variety
};

// 🎯 Game 2: Generate random helpers (copy symbol pattern)
const generateRandomHelpers = (missionId) => {
  const mission = HELPER_MISSIONS[missionId];
  if (!mission?.helpers) return [];
  const shuffled = [...mission.helpers].sort(() => 0.5 - Math.random());
  return shuffled.slice(0, 3); // Take 3 of 4 helpers
};

// 🎯 Game 2: Start Mission (copy startScenario)
const startMission = (missionIndex = 0) => {
  console.log(`🎯 Starting mission ${missionIndex + 1} with problem speech`);
  
  const missions = sceneState.game2SelectedMissions || generateRandomMissions();
  const missionId = missions[missionIndex];
  const missionData = HELPER_MISSIONS[missionId];
  const missionHelpers = generateRandomHelpers(missionId);
  
  sceneActions.updateState({
    game2CurrentMission: missionIndex,
    game2CurrentMissionData: missionData,
    game2MissionHelpers: missionHelpers,
    phase: SCENE_PHASES.MISSION_UNCERTAINTY,
    game2MissionPhase: 'uncertainty',
    game2ShowProblemSpeech: true,
    game2HelpersVisible: 'none',
    game2HelpersClickable: false,
    game2CurrentBackground: 'before'
  });
  
  setTimeout(() => {
    sceneActions.updateState({
      game2ShowProblemSpeech: false,
      game2ProblemSpeechShown: true,
      phase: SCENE_PHASES.MISSION_CLARITY,
      game2MissionPhase: 'clarity',
      game2HelpersVisible: 'clear',
      game2HelpersClickable: true
    });
  }, 3000);
};

// 🎯 Game 2: Helper Click (copy handleSymbolClick)
const handleHelperClick = (helperId, helperData) => {
  console.log(`🤖 Helper selected: ${helperId}`);
  
  sceneActions.updateState({
    game2SelectedHelper: helperId,
    game2SelectedHelperData: helperData,
    game2HelperPopupVisible: true,
    game2HelperMessage: helperData.phrase,
    phase: SCENE_PHASES.MISSION_CHOOSING,
    game2MissionPhase: 'choosing'
  });
...
const handleHelperConfirm = () => {
  if (!sceneState.game2SelectedHelperData) return;
  
  console.log(`✅ Helper activated: ${sceneState.game2SelectedHelperData.name}`);
  
  sceneActions.updateState({
    game2HelperPopupVisible: false,
    phase: SCENE_PHASES.MISSION_SUCCESS,
    game2MissionPhase: 'success'
  });
  
  setShowSparkle('helper-animation');
};

// ✅ NEW: Add this function after handleHelperConfirm
const onHelperAnimationComplete = () => {
  console.log('🎭 Helper animation complete - starting blessing sequence');
  
  // Step 1: Show Ganesha + Helper blessing (3 seconds)
  sceneActions.updateState({
    phase: SCENE_PHASES.GANESHA_GOLDEN_BLESSING,
    ganeshaPresent: true,
    goldenChildActive: true,
    divineMessage: "Beautiful choice, my golden helper! You chose with wisdom!"
  });
  
  // Step 2: Show thank you + change image (after 3 seconds)
  setTimeout(() => {
    sceneActions.updateState({
      game2CurrentBackground: 'after', // ✅ Change image before → after
      missionGratitudeVisible: true,   // ✅ Show thank you message
      phase: SCENE_PHASES.GAME2_MISSION_COMPLETE
    });
  }, 3000);
  
  // Step 3: Return to helper selection (after 6 seconds total)
  setTimeout(() => {
    sceneActions.updateState({
      phase: SCENE_PHASES.MISSION_CLARITY,
      game2MissionPhase: 'clarity',
      ...
      goldenChildActive: false,
      divineMessage: null,
      // Keep helpers visible for next selection
      game2HelpersVisible: 'clear',
      game2HelpersClickable: true
    });
  }, 6000);
...
// ✅ ADD IMAGE 1 HERE:
const showGaneshaGoldenBlessing = () => {
  sceneActions.updateState({
    phase: SCENE_PHASES.GAME2_GANESHA_GOLDEN_BLESSING,
    ganeshaPresent: true,
    goldenChildActive: true,
    divineMessage: "Beautiful choice, my golden helper! You chose with wisdom!"
  });
  
  // Show both together - divine partnership
  setTimeout(() => {
    sceneActions.updateState({
      phase: SCENE_PHASES.GAME2_MISSION_COMPLETE,
...
// 🎯 Game 2: Complete Mission (copy completeCurrentScenario)
const completeCurrentMission = () => {
  const nextMission = (sceneState.game2CurrentMission || 0) + 1;
  const missionsCompleted = (sceneState.game2MissionsCompleted || 0) + 1;
  
  sceneActions.updateState({
    game2MissionsCompleted: missionsCompleted,
    stars: (sceneState.stars || 6) + 1
  });
  
  if (missionsCompleted >= 3) {
    setTimeout(() => completeGame2(), 2000);
  } else {
    setTimeout(() => startMission(nextMission), 3000);
  }
};

// 🎯 Game 2: Complete All Missions
const completeGame2 = () => {
  console.log('🌟 All missions completed!');
  
  sceneActions.updateState({
    phase: SCENE_PHASES.SARVADA_LEARNING,
    showAlwaysText: true,
    completed: true,
    stars: 10
  });
...
onClick={() => {
              ...
              sceneActions.updateState({
                ...
                phase: SCENE_PHASES.GAME2_TRANSFORMATION,
                game2SelectedMissions: missions,
                // ✅ ADD: Set mission data immediately
                game2CurrentMissionData: firstMissionData,
                game2CurrentBackground: 'before',
                stars: 8
              });
              setTimeout(() => startMission(0), 500);
            }}
            style={{
              position: 'absolute',
              top: '40px',
...
{sceneState.phase === SCENE_PHASES.GAME2_TRANSFORMATION && (
  <div className="transformation-text" style={{
    position: 'absolute', top: '50%', left: '50%',
    transform: 'translate(-50%, -50%)', fontSize: '32px',
    color: '#FFD700', zIndex: 15
  }}>
    You are now Ganesha's Golden Helper!
  </div>
)}

{/* Game 2 - Mission Window with CSS classes */}
{(sceneState.phase === SCENE_PHASES.MISSION_UNCERTAINTY ||
  sceneState.phase === SCENE_PHASES.MISSION_CLARITY ||
  sceneState.phase === SCENE_PHASES.MISSION_CHOOSING ||
  sceneState.phase === SCENE_PHASES.MISSION_SUCCESS ||
  sceneState.phase === SCENE_PHASES.GAME2_TRANSFORMATION) && (
  <>
    {/* Mission Window */}
    <div className="scenario-window">
      <div 
        className={`mission-background ${sceneState.game2CurrentBackground || 'before'}`}
        style={{
          width: '100%',
          height: '100%',
      ...
      {/* Problem Speech */}
      {sceneState.game2ShowProblemSpeech && sceneState.game2CurrentMissionData && (
        <div className="character-problem-speech">
          <p>{sceneState.game2CurrentMissionData.intro}</p>
        </div>
      )}
    </div>

    {/* Helpers (COPY symbols pattern) */}
    {sceneState.game2HelpersVisible !== 'none' && 
      sceneState.game2MissionHelpers.map((helper, index) => (
        <div 
          key={helper.id} 
          className={`cave-symbol-individual position-${index + 1} ${sceneState.game2HelpersVisible}`}
        >
          <ClickableElement
            id={`helper-${helper.id}`}
            onClick={() => handleHelperClick(helper.id, helper)}
          >
            <div className="cave-symbol-item helper-item">
              <img src={helper.image} alt={helper.name} />
              <span className="symbol-name-text">{helper.name}</span>
            </div>
          </ClickableElement>
        </div>
      ))
    ...
    {/* Helper Popup */}
    {sceneState.game2HelperPopupVisible && sceneState.game2SelectedHelperData && (
      <div className="helper-popup-center">
        <div className="helper-popup-card">
          <img src={sceneState.game2SelectedHelperData.image} />
          <h3>{sceneState.game2SelectedHelperData.name}</h3>
          <div className="helper-speech-bubble">
            <p>{sceneState.game2HelperMessage}</p>
          </div>
          <ClickableElement onClick={handleHelperConfirm}>
            <div className="helper-activation-button">
              Tap me to activate!
            </div>
          </ClickableElement>
        </div>
...
{/* Signature Animation */}
    {sceneState.phase === SCENE_PHASES.MISSION_SUCCESS && sceneState.game2SelectedHelperData && (
      <HelperSignatureAnimation
        helperId={sceneState.game2SelectedHelperData.id}
        fromPosition={{ x: 20, y: 20 }}
        toPosition={{ x: 50, y: 50 }}
        onAnimationComplete={onHelperAnimationComplete} // ✅ Use new function
      />
    )}
  </>
)}

{/*ADD: Divine partnership moment (copy Game 1's Ganesha pattern)*/}
{sceneState.phase === SCENE_PHASES.GANESHA_GOLDEN_BLESSING && (
  <div className="divine-partnership">
    {/* Golden Child Helper */}
    <div className="golden-child-helper">
      <div className="golden-child-avatar">
        <img 
          src={sceneState.selectedCharacter === 'boy' ? boyPreview : girlPreview} 
          alt="Golden Helper Child"
        />
        <div className="golden-helper-aura"></div>
      </div>
      <div className="helper-badge">Ganesha's Helper</div>
    </div>

    {/* Ganesha's Blessing */}
    <div className="ganesha-divine-blessing">
      <div className="ganesha-emergence-glow"></div>
      <div className="emerging-ganesha blessing">
        <img src={ganeshaIcon} alt="Divine Ganesha Blessing" />
      </div>
      {sceneState.divineMessage && (
        <div className="divine-blessing-speech">
          <p>{sceneState.divineMessage}</p>
        </div>
      )}
    </div>

    {/* Visual Connection Line */}
    <div className="divine-connection-line"></div>
    
    {/* Success Aura */}
    <div className="divine-success-aura"></div>
  </div>
)}

{/* Mission Gratitude Bubble - After Helper Animation */}
{sceneState.missionGratitudeVisible && sceneState.game2CurrentMissionData && (
  <div className="mission-gratitude-bubble">
    <p>{sceneState.game2CurrentMissionData.gratitude}</p>
  </div>
)}
