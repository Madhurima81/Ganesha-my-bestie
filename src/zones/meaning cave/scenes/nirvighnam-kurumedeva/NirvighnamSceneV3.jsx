...
const getBridgeRockPosition = (displayIndex) => {
  ...
  // ✅ CHANGED: 3 positions instead of 5
  const positions = [
...
useEffect(() => {
  ...
  setTimeout(() => {
    ...
    // ✅ NEW: Handle animated text states during reload
    if (sceneState.showObstacleText || 
        (sceneState.learnedWords?.nirvighnam?.learned && 
         sceneState.phase === CAVE_PHASES.NIRVIGHNAM_LEARNING)) {
      console.log('🔄 NIRVIGHNAM: Resuming during Nirvighnam text animation - skipping to Door 2');
      sceneActions.updateState({
    ...
    if (sceneState.showDivineHelpText || 
      ...
      console.log('🔄 NIRVIGHNAM: Resuming during Kurume Deva text animation - skipping to fireworks');
      sceneActions.updateState({
    ...
    // ✅ UPDATED: Simplified popup handling (remove card states)
    ...
    // ✅ ADD: Handle incomplete Door 2 completion and bridge start
    if (sceneState.door2Completed && !sceneState.bridgeBuildingStarted && 
      ...
      console.log('🔄 NIRVIGHNAM: Door 2 completed but bridge not started - resuming bridge intro');
      sceneActions.updateState({
    ...
    // ✅ ADD: Handle incomplete Door 1 completion and crystal fog start
    if (sceneState.door1Completed && !sceneState.crystalFogStarted && 
        ...
        !sceneState.currentPopup) {
      console.log('🔄 NIRVIGHNAM: Door 1 completed but crystal fog not started - resuming crystal fog intro');
      sceneActions.updateState({
    ...
    // ✅ ADD: Handle animated text states during reload (AFTER other state checks)
...
const handleDoor1Complete = () => {
  ...
  sceneActions.updateState({
    phase: CAVE_PHASES.CRYSTAL_FOG_ACTIVE,  // ← Game starts immediately!
    crystalFogStarted: true                  // ← Game ready to play!
  });
...
const handleDoor2Complete = () => {
  ...
  // ✅ STEP 4: Start game IMMEDIATELY, clear sparkles later
  sceneActions.updateState({
    phase: CAVE_PHASES.BRIDGE_BUILDING_ACTIVE,  // ← Game starts immediately!
    bridgeBuildingStarted: true                  // ← Game ready to play!
  });
...
const handleCrystalFogComplete = () => {
  ...
  sceneActions.updateState({
    ...
    showObstacleText: true, // ✨ NEW: Show animated text
    progress: {
      ...sceneState.progress,
      percentage: 50,
      starsEarned: 3
...
const completeNirvighnamLearning = () => {
  console.log('📜 Nirvighnam learned - updating sidebar only');
...
const completeBridgeBuilding = () => {
  ...
  sceneActions.updateState({
    ...
    showDivineHelpText: true  // ✨ NEW: Show animated text
  });
...
const startKurumedevaLearning = () => {
  console.log('📜 Kurume Deva learned - updating sidebar only');
...
{/* ✨ NEW: Nirvighnam to Sidebar Effect */}
...
{/* ✨ NEW: Kurume Deva to Sidebar Effect */}
