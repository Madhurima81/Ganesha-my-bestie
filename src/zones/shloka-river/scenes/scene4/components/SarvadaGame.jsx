...
const sarvadaGame = ({
  ...
  console.log('🌟 sarvadaGame render:', { 
    isActive, 
    hideElements,
    isReload,
    initialCurrentRound,
  ...
  const [currentPhase, setCurrentPhase] = useState('sarvada');
  ...
  useEffect(() => {
    ...
    return () => {
      console.log('🧹 sarvadaGame: Component unmounting - cleaning up');
      isComponentMountedRef.current = false;
      clearAllTimers();
      
      // Cleanup global references
      if (window.sarvadaGame) {
        window.sarvadaGame.isReady = false;
        delete window.sarvadaGame.startGame;
        delete window.sarvadaGame.clearCompletionState;
      }
  ...
  useEffect(() => {
    if (!isActive) {
      console.log('🛑 sarvadaGame: Component deactivated - clearing timers');
      clearAllTimers();
      ...
      if (window.sarvadaGame) {
        window.sarvadaGame.isReady = true;
      }
    }
  }, [isActive]);
...
const syllableFileMap = {
  ...
  const playCompleteWord = () => {
    ...
    try {
playAudio('/audio/words/sarvada.mp3', 0.9);
    } catch (error) {
  ...
  const getSequenceForRound = (round) => {
    return SYLLABLE_SEQUENCES.sarvada[round] || [];
  };
  ...
  const startNewRound = (roundNumber) => {
    ...
    console.log(`🌟 Starting sarvada Round ${roundNumber} with sequence:`, sequence);
  ...
  useEffect(() => {
    ...
    if (isReload && initialCurrentSequence.length > 0) {      
      console.log('🔄 RELOAD: Restoring sarvada game state');
      ...
      if (initialGamePhase === 'celebration') {
        ...
        safeSetTimeout(() => {
          ...
          if (initialCurrentRound < 2) { // UPDATED: max round is now 3
  ...
  useEffect(() => {
    ...
    window.sarvadaGame.isReady = true;
    window.sarvadaGame.clearCompletionState = () => {
      if (isComponentMountedRef.current) {
        setGamePhase('waiting');
        setSequenceItemsShown(0);
        console.log('🧹 sarvada game completion state cleared');
      }
    };
    
    console.log('🌐 sarvadaGame: Global functions exposed and ready flag set');
    
    return () => {
      if (window.sarvadaGame) {
        window.sarvadaGame.isReady = false;
        delete window.sarvadaGame.clearCompletionState;
        console.log('🧹 sarvadaGame: Global functions cleaned up');
      }
    };
  }, [isActive]);
  ...
  const handleSequenceSuccess = () => {
    ...
    safeSetTimeout(() => {
      ...
      if (currentRound < 2) { // UPDATED: max round is now 3
        console.log(`➡️ AUTO-ADVANCE: Starting round ${currentRound + 1} of sarvada`);
  ...
  const handlePhaseComplete = () => {
    ...
    saveGameState({
      ...
      lastCompletedPhase: 'sarvada'
    });
    
    if (onPhaseComplete) {
      onPhaseComplete('sarvada');
    }
  };
  ...
  const getStatusMessage = () => {
    ...
    return `Round ${currentRound} - SARVADA`;
  };
  ...
  const renderAnimal = (syllable, index) => {
    ...
    const animalData = ELEMENT_DATA.sarvada.animals
[index];
  ...
  const renderItem = (syllable, index) => {
    ...
    const itemData = ELEMENT_DATA.sarvada.items[index];
  ...
  const renderDecor = (syllable, index) => {
    const decorData = ELEMENT_DATA.sarvada.decor[index];
