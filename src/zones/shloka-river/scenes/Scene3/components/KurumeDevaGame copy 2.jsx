...
// Mobile-first responsive scaling hook
const useResponsiveScaling = () => {
  ...
  useEffect(() => {
    const updateMetrics = () => {
      ...
      // MOBILE-FIRST: iPhone 12 Pro reference (390x844)
      const MOBILE_REFERENCE = { width: 390, height: 844 };
      
      // Calculate scale factors (scaling UP from mobile baseline)
      const widthScale = width / MOBILE_REFERENCE.width;
      const heightScale = height / MOBILE_REFERENCE.height;
      const baseScale = Math.min(widthScale, heightScale);
      
      // Allow generous scaling up from mobile baseline
      ...
      if (width >= 1400) breakpoint = 'large';
      
      const isStandalone = window.matchMedia('(display-mode: standalone)').matches;
      const isTouch = 'ontouchstart' in window;
      
      setMetrics({ scaleFactor, breakpoint, isTouch, isStandalone });
    };
...
// Mobile-first element data generator
const createResponsiveElements = (scaleFactor, breakpoint, isTouch) => {
  // Scale UP from mobile baseline instead of DOWN from desktop
  const deviceMultipliers = {
    mobile: { animals: 1.0, items: 1.0, decor: 1.0 },      // Base reference size
    tablet: { animals: 1.3, items: 1.2, decor: 1.6 },      // Scale up for tablets  
    desktop: { animals: 1.5, items: 1.4, decor: 2.0 },     // Scale up for desktops
    large: { animals: 1.7, items: 1.6, decor: 2.4 }        // Scale up for large screens
  };
  
  const mult = deviceMultipliers[breakpoint];
  
  // Mobile-optimized base sizes (designed for iPhone 12 Pro)
  const baseSizes = {
    animals: 55,    // Perfect touch target for mobile
    items: 35,      // Clear visibility on mobile
    decor: 160      // Balanced garland size on mobile
  };
  
  const calcSize = (baseSize, elementType, individualMultiplier = 1) => {
    const finalSize = Math.round(baseSize * individualMultiplier * scaleFactor * mult[elementType]);
    // Ensure minimum touch targets for children
    return (isTouch && elementType === 'animals') ? Math.max(finalSize, 44) : finalSize;
  };
    ...
    kurumeDeva: {
      animals: [
        { 
          ...
          size: { 
            width: `${calcSize(baseSizes.animals, 'animals', 2.0)}px`, 
            height: `${calcSize(baseSizes.animals, 'animals', 2.0)}px` 
          } 
        },
        { 
          id: 'animal2-ru', 
          ...
          size: { 
            width: `${calcSize(baseSizes.animals, 'animals', 3.0)}px`, 
            height: `${calcSize(baseSizes.animals, 'animals', 3.0)}px` 
          } 
        },
        { 
          id: 'animal3-me', 
          ...
          size: { 
            width: `${calcSize(baseSizes.animals, 'animals', 3.05)}px`, 
            height: `${calcSize(baseSizes.animals, 'animals', 3.05)}px` 
          } 
        },
        { 
          id: 'animal4-de', 
          ...
          size: { 
            width: `${calcSize(baseSizes.animals, 'animals', 1.45)}px`, 
            height: `${calcSize(baseSizes.animals, 'animals', 1.45)}px` 
          } 
        },
        { 
          id: 'animal5-va', 
          ...
          size: { 
            width: `${calcSize(baseSizes.animals, 'animals', 1.2)}px`, 
            height: `${calcSize(baseSizes.animals, 'animals', 1.2)}px` 
          } 
        }
      ],
      items: [
        { 
          ...
          size: { 
            width: `${calcSize(baseSizes.items, 'items', 2.3)}px`, 
            height: `${calcSize(baseSizes.items, 'items', 2.3)}px` 
          } 
        },
        { 
          id: 'item2-ru', 
          ...
          size: { 
            width: `${calcSize(baseSizes.items, 'items', 2.1)}px`, 
            height: `${calcSize(baseSizes.items, 'items', 2.1)}px` 
          } 
        },
        { 
          id: 'item3-me', 
          ...
          size: { 
            width: `${calcSize(baseSizes.items, 'items', 1.2)}px`, 
            height: `${calcSize(baseSizes.items, 'items', 1.2)}px` 
          } 
        },
        { 
          id: 'item4-de', 
          ...
          size: { 
            width: `${calcSize(baseSizes.items, 'items', 1.3)}px`, 
            height: `${calcSize(baseSizes.items, 'items', 1.3)}px` 
          } 
        },
        { 
          id: 'item5-va', 
          ...
          size: { 
            width: `${calcSize(baseSizes.items, 'items', 1.4)}px`, 
            height: `${calcSize(baseSizes.items, 'items', 1.4)}px` 
          } 
        }
      ],
      decor: [
        { 
          ...
          size: { 
            // KEY FIX: 160px mobile baseline becomes 320px desktop (no more cutoff)
            width: `${calcSize(baseSizes.decor, 'decor', 2.8)}px`, 
            height: `${calcSize(baseSizes.decor, 'decor', 2.8)}px` 
          } 
        },
        { 
          id: 'decor2-ru', 
          ...
          size: { 
            width: `${calcSize(baseSizes.decor, 'decor', 0.5)}px`, 
            height: `${calcSize(baseSizes.decor, 'decor', 0.5)}px` 
          } 
        },
        { 
          id: 'decor3-me', 
          ...
          size: { 
            width: `${calcSize(baseSizes.decor, 'decor', 0.8)}px`, 
            height: `${calcSize(baseSizes.decor, 'decor', 0.8)}px` 
          } 
        },
        { 
          id: 'decor4-de', 
          ...
          size: { 
            width: `${calcSize(baseSizes.decor, 'decor', 0.9)}px`, 
            height: `${calcSize(baseSizes.decor, 'decor', 0.9)}px` 
          } 
        },
        { 
          id: 'decor5-va', 
          ...
          size: { 
            width: `${calcSize(baseSizes.decor, 'decor', 1.0)}px`, 
            height: `${calcSize(baseSizes.decor, 'decor', 1.0)}px` 
          } 
        }
      ]
    }
...
const KurumeDevaGame = ({
  ...
  const { scaleFactor, breakpoint, isTouch, isStandalone } = useResponsiveScaling();
  const ELEMENT_DATA = createResponsiveElements(scaleFactor, breakpoint, isTouch);
...
const uiScale = breakpoint === 'mobile' ? 1 : 
  ...
  console.log('Mobile-First Scale Info:', { scaleFactor, breakpoint, isTouch, isStandalone });
...
// UPDATE the main button style object:
style={{
  position: 'absolute',
  left: position.left,
  top: position.top,
  ...
  zIndex: 20,
  borderRadius: '50%', // Makes it feel more natural
  ...finalStyle
}}
...
{!hideElements && gamePhase !== 'phase_complete' && (
  <div style={{
    ...
    textAlign: 'center',
    zIndex: 50,
    maxWidth: `${Math.round(200 * uiScale)}px`,
...
{/* Responsive Round Selector Buttons */}
{!hideElements && (gamePhase === 'waiting' || gamePhase === 'animal_clicking') && (
  <div style={{
    position: 'absolute',
    top: `${Math.round(50 * uiScale)}px`,
    left: `${Math.round(135 * uiScale)}px`,
    display: 'flex',
    gap: `${Math.round(4 * uiScale)}px`,
    zIndex: 45
  }}>
    {[1, 2, 3, 4].map(round => (
      <button key={round} onClick={() => jumpToRound(currentPhase, round)} style={{
        ...
        cursor: 'pointer',
        fontWeight: '500',
        minWidth: `${Math.round(18 * uiScale)}px`
      }}>
        {round}
      </button>
    ))}
