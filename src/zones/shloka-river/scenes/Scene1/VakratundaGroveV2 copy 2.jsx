...
import GaneshaBlessing from './components/GaneshaBlessing';
...
const VakratundaGroveContent = ({
  ...
  // NEW: Blessing system state
  const [showGaneshaBlessing, setShowGaneshaBlessing] = useState(false);
  const [currentBlessingPower, setCurrentBlessingPower] = useState(null);
  const [earnedPowers, setEarnedPowers] = useState([]);
  
  // NEW: Rescue mission state (we'll implement this next)
...
// Update your existing handlePhaseComplete function
  const handlePhaseComplete = (phase) => {
    console.log(`Phase ${phase} completed!`);
    
    if (phase === 'vakratunda') {
      // Instead of directly showing recording, show Ganesha blessing
      sceneActions.updateState({
        learnedWords: { ...sceneState.learnedWords, vakratunda: true },
        learnedSyllables: {
          ...sceneState.learnedSyllables,
        ...
        phase: PHASES.VAKRATUNDA_BLESSING  // NEW: Go to blessing phase
      });
      
      // Show blessing ceremony
      setCurrentBlessingPower('vakratunda');
      setShowGaneshaBlessing(true);
      
    } else if (phase === 'mahakaya') {
      // Same pattern for Mahakaya
      sceneActions.updateState({
        learnedWords: { ...sceneState.learnedWords, mahakaya: true },
        learnedSyllables: {
          ...sceneState.learnedSyllables,
        ...
        phase: PHASES.MAHAKAYA_BLESSING  // NEW: Go to blessing phase
      });
      
      // Show blessing ceremony
      setCurrentBlessingPower('mahakaya');
      setShowGaneshaBlessing(true);
    }
  };

  // NEW: Handle blessing completion
  const handleBlessingComplete = (blessingData) => {
    console.log('Blessing completed:', blessingData);
    
    setShowGaneshaBlessing(false);
    setCurrentBlessingPower(null);
    
    // Add power to collection
    const newPower = {
      name: blessingData.powerConfig.name,
      icon: blessingData.powerConfig.icon,
      color: blessingData.powerConfig.color,
      id: blessingData.power
    };
    
    setEarnedPowers(prev => [...prev, newPower]);
    
    // Show celebration
    setShowSparkle(`${blessingData.power}-blessed`);
    
    // Continue to next phase or rescue mission
    setTimeout(() => {
      setShowSparkle(null);
      
      if (blessingData.power === 'vakratunda') {
        // Continue to Mahakaya memory game
        sceneActions.updateState({ phase: PHASES.MEMORY_GAME_ACTIVE });
        // You can add rescue mission here later
        
      } else if (blessingData.power === 'mahakaya') {
        // Scene complete
        sceneActions.updateState({
          phase: PHASES.SCENE_COMPLETE,
          stars: 5,
...
{/* {renderProgressCounter()} */}
            ...
            />
             ...
             {/* NEW: Ganesha Blessing Component */}
            {showGaneshaBlessing && (
              <GaneshaBlessing
                power={currentBlessingPower}
                characterGender="boy" // You can make this dynamic based on user selection
                onBlessingComplete={handleBlessingComplete}
                ganeshaImage="/images/ganesha/ganesha_with_headphones.png" // Your beautiful image
                characterImage="/images/characters/boy_receiving_blessing.png" // Optional
                show={showGaneshaBlessing}
              />
            )}
...
/>
 ...
 {/* Enhanced Sparkle Effects for Blessings */}
            {showSparkle === 'vakratunda-blessed' && (
              <SparkleAnimation
                type="blessing"
                count={50}
                color="#FFD700"
