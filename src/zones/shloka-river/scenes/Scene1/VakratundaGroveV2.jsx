...
const handleSaveAnimal = () => {
  ...
  // FIXED: Reset rescue mission state completely
  setRescuePhase('problem');
  setShowRescueMission(false); // Hide first
  
  // Show after brief delay to ensure clean state
  setTimeout(() => {
...
if (currentRescueWord === 'vakratunda') {
    ...
    // FIXED: Show Mahakaya story instead of directly starting game
    setTimeout(() => {
      setShowMahakayaStory(true);
    }, 500);
  ...
  // Go directly to final fireworks, not choice buttons
  setShowRescueMission(false);
  setShowChoiceButtons(false);
  setShowGaneshaBlessing(false);
...
const handleContinueLearning = () => {
  ...
  if (currentPracticeWord === 'vakratunda') {
    ...
    // CLEAR ALL VAKRATUNDA STATES IMMEDIATELY
    setShowGaneshaBlessing(false);
    setShowSparkle(null);           
    setShowWordCelebration(false);  
    setVakratundaPowerGained(true);
    
    // NEW: Clear memory game states that might show "mastered"
    sceneActions.updateState({      
      currentPopup: null,
      phase: PHASES.MEMORY_GAME_ACTIVE,
      gameCoachState: null  // ADD THIS LINE
    });
    
    // NEW: Force memory game to clear any completion states
    if (window.sanskritMemoryGame && window.sanskritMemoryGame.clearCompletionState) {
      window.sanskritMemoryGame.clearCompletionState();
    }
    
    // Show Mahakaya story instead of directly starting game
    setTimeout(() => {
      setShowMahakayaStory(true);
    }, 200);
  }
...
{showMahakayaStory && (
  ...
  onClick={() => {
    ...
    // Proper transition to Mahakaya phase
    sceneActions.updateState({ 
      phase: PHASES.MEMORY_GAME_ACTIVE,
      currentPopup: null 
    });
    
    // Start Mahakaya immediately after hiding story
    setTimeout(() => {
      if (window.sanskritMemoryGame?.startMahakayaPhase) {
        console.log('Calling startMahakayaPhase after story');
        window.sanskritMemoryGame.startMahakayaPhase();
      } else {
        console.error('sanskritMemoryGame.startMahakayaPhase not available');
      }
    }, 300);
  }}
        style={{
...
{(showGaneshaBlessing || showChoiceButtons) && 
 !showRescueMission &&           // This line should already exist
 !showMahakayaStory && 
 !showRecording &&               // ADD THIS LINE
 sceneState.phase !== PHASES.SCENE_COMPLETE && (  // ADD THIS LINE
  <div style={{
    position: 'absolute',
    right: '15%',
    bottom: '20%',
...
{showWordCelebration && (
  <div style={{
    ...
    {/* Show previous word too when mahakaya is complete */}
    {(blessingWord || currentPracticeWord) === 'mahakaya' && (
      <div style={{
        fontSize: '32px',
        fontWeight: 'bold',
        color: '#FFD700',
        textShadow: '0 0 15px rgba(255, 215, 0, 0.6)',
        animation: 'wordCelebrationSecondary 3s ease-in-out',
        opacity: 0.8
      }}>
        VAKRATUNDA
      </div>
...
onComplete={() => {
  ...
  try {
    ...
    console.error('âŒ Error in fireworks completion:', error);
...
@keyframes wordCelebrationSecondary {
