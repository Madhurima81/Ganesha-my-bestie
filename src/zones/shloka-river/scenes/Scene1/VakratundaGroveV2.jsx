    ...
    <ErrorBoundary>
      <SceneManager
        ...
        initialState={{
          ...
          unlockedApps: {
    // No previous apps - this is Scene 1
  },
...
const VakratundaGroveContent = ({
 ...
 // In your VakratundaGroveV3.jsx component, update this line:
...
const { resetScene } = useSceneReset(
  ...
  'shloka-river', 
  'vakratunda-grove',
  getSceneResetConfig('vakratunda-grove'),
  {
  ...
  useEffect(() => {
    ...
    return () => {
      if (window.SimplifiedMemoryGame) {
        delete window.SimplifiedMemoryGame;
      }
    };
  }, []);
...
const handleWordPlay = (word) => {
  ...
  const startPracticeRound = (word, round) => {
    ...
    if (window.SimplifiedMemoryGame && window.SimplifiedMemoryGame.startPracticeMode) {
      window.SimplifiedMemoryGame.startPracticeMode(word, round);
    }
...
const startBlessingAnimation = async () => {
  try {
    ...
    // Update unlocked apps in scene state (like discoveredSymbols)
sceneActions.updateState({
  unlockedApps: {
  ...
  // Assets for both phases
  getBabyElephantImage={getBabyElephantImage}    // Vakratunda elephants
  getAdultElephantImage={getAdultElephantImage}  // Mahakaya elephants
  getBudImage={getBudImage}                       // Vakratunda initial
  getLotusImage={getLotusImage}                   // Vakratunda rewards
  getSeedImage={getSeedImage}                     // Mahakaya initial
  getFlowerImage={getFlowerImage}                 // Mahakaya rewards
  ...
  initialPlayerInput={sceneState.memoryGameState?.playerInput || []}
  initialCurrentSequence={sceneState.memoryGameState?.currentSequence || []}
  initialVisualRewards={sceneState.memoryGameState?.visualRewards || {}}
  initialActivatedElephants={sceneState.memoryGameState?.activatedElephants || {}}
...
<div style={{
  ...
  SKIP TO MAHAKAYA
          ...
          }} // Replace your existing "Start Fresh" button click handler with this:
...
onClick={() => {
  if (confirm('Start this Sanskrit scene from the beginning? You will lose current progress.')) {
    ...
    if (typeof setVakratundaPowerGained !== 'undefined') setVakratundaPowerGained(false);
    ...
    // STEP 5: FIXED - Clear SIMPLIFIED memory game completion state (not sanskritMemoryGame)
    if (window.simplifiedMemoryGame && window.simplifiedMemoryGame.clearCompletionState) {
      window.simplifiedMemoryGame.clearCompletionState();
    }
    
    // STEP 6: FIXED - Force SimplifiedMemoryGame reset (not sanskritMemoryGame)
    const forceMemoryGameReset = () => {
      if (window.simplifiedMemoryGame) {
        // Clear visual rewards - this is the key for SimplifiedMemoryGame
    ...
    setTimeout(() => {
      ...
      console.log('Sanskrit scene state reset complete');
    }, 150);
  }
}}>
            Start Fresh
