...
useEffect(() => {
  ...
  setTimeout(() => {
    // 🎯 PRIORITY 1: HANDLE COMPLETION SCREEN FIRST (before any popups)
   // 🎯 PRIORITY 1: HANDLE COMPLETION SCREEN FIRST (but check Play Again first)
if (sceneState.showingCompletionScreen) {
  ...
  return;
    ...
    // 2. HANDLE SYMBOL DISCOVERY STATES (only if not completed)
    if (sceneState.symbolDiscoveryState) {
    ...
    // 3. HANDLE SIDEBAR HIGHLIGHT STATES  
    ...
    // 4. HANDLE REGULAR POPUP STATES (only if not completed)
...
/*onReplay={() => {
  console.log('🔧 POND PLAY AGAIN: Resetting scene but keeping resume tracking');
  
  // 1. Enhanced GameCoach clearing
  if (clearManualCloseTracking) {
    clearManualCloseTracking();
    console.log('✅ POND PLAY AGAIN: GameCoach manual tracking cleared');
  }
  if (hideCoach) {
    hideCoach();
    console.log('✅ POND PLAY AGAIN: GameCoach hidden');
  }
  
  // ✅ ADD: Enhanced GameCoach timeout (500ms)
  setTimeout(() => {
    console.log('🎭 POND PLAY AGAIN: Forcing GameCoach fresh start');
    if (clearManualCloseTracking) {
      clearManualCloseTracking();
      console.log('🎭 POND PLAY AGAIN: GameCoach cleared again after delay');
    }
  }, 500);
  
  // ✅ ADD: Clear temp storage and preserve scene tracking
  const profileId = localStorage.getItem('activeProfileId');
  if (profileId) {
    ...
    console.log('✅ POND PLAY AGAIN: Current scene preserved for resume tracking');
  }
  ...
  setTimeout(() => {
    ...
    setTimeout(() => {
      ...
      sceneActions.updateState({
        ...
        // ✅ CRITICAL: GameCoach reset flags
        welcomeShown: false,
        isReloadingGameCoach: false,
        gameCoachState: null,
        lastGameCoachTime: 0,
        
        // ✅ CRITICAL: UI reset flags
        currentPopup: null,
        symbolDiscoveryState: null,
        sidebarHighlightState: null,
        showingCompletionScreen: false,
      ...
      console.log('✅ POND PLAY AGAIN: Scene state reset complete');
    }, 200);
  }, 200);
  
  // ✅ ADD: Aggressive cleanup for intermittent issues (1000ms)
  setTimeout(() => {
    const profileId = localStorage.getItem('activeProfileId');
    if (profileId) {
      const tempKey = `temp_session_${profileId}_symbol-mountain_pond`;
      ...
      console.log('🧪 POND AGGRESSIVE CLEAR: Removed all session storage');
    }
    ...
    console.log('✅ POND PLAY AGAIN: Aggressive cleanup complete');
  }, 1000);
}}*/
