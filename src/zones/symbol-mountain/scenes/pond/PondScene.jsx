...
// Update handleCloseCelebration function
const handleCloseCelebration = () => {
  setShowCelebration(false);
  
  // Continue game progression after celebration closes
  if (gameStage === 2 && goldenLotusVisible) {
    // Golden lotus is already visible (from the new flow), don't duplicate
    return;
  }
...
useEffect(() => {
 ...
 // Update the useEffect that handles lotus blooming
useEffect(() => {
  ...
  if (newCount > 0 && newCount < 3) {
    ...
    // Immediately show golden lotus when all 3 bloom
    safeSetTimeout(() => {
      // Add sparkle effect
...
// Update handleCloseScroll function
const handleCloseScroll = () => {
  ...
  else if (currentPopupImage === popup1 && bloomedCount === 3) {
    // After lotus scroll, show lotus celebration
    // Golden lotus is already visible now
    safeSetTimeout(() => {
