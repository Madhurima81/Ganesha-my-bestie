    ...
    <ErrorBoundary>
      <SceneManager
        ...
        initialState={{
          ...
          // ENHANCED RELOAD SYSTEM - Fixed GameCoach handling
          currentPopup: null,  // 'lotus_info', 'lotus_card', 'elephant_info', 'trunk_card', 'final_fireworks'
          
          // NEW: Separate GameCoach reload tracking
          gameCoachState: null,  // 'lotus_wisdom', 'elephant_wisdom', 'mastery_wisdom'
          isReloadingGameCoach: false,  // Flag to prevent double triggers
          
          // ADD THESE BACK FOR GAMEWELCOMESCREEN
          stars: 0,           
          completed: false,   
          progress: {
            percentage: 0,
...
const PondSceneContent = ({ 
  ...
  useEffect(() => {
    if (previousVisibilityRef.current && !isVisible && pendingAction) {
      ...
      }, 1000);  // Reduced delay from 2500ms to 1000ms
  ...
  // FIXED: GameCoach useEffect with precise reload prevention
  useEffect(() => {
    if (!sceneState || !showMessage) return;
    
    // Only block during active reload process, not when gameCoachState exists from previous session
    ...
    if (matchingMessage) {
      ...
      const messageAlreadyShown = 
        (matchingMessage.id === 'lotus_wisdom' && sceneState.lotusWisdomShown) ||
        (matchingMessage.id === 'elephant_wisdom' && sceneState.elephantWisdomShown) ||
      ...
      const timer = setTimeout(() => {
        ...
        switch (matchingMessage.id) {
          ...
          case 'lotus_wisdom':
            ...
            // Set pending action for normal flow too
    ...
    sceneState?.isReloadingGameCoach,  // Only block during active reload
    showMessage
  ]);
  ...
  useEffect(() => {
    ...
    if ((sceneState.phase === PHASES.ALL_BLOOMED || 
      ...
      sceneActions.updateState({ currentPopup: 'lotus_info' });
  ...
  // COMPLETELY REWRITTEN RELOAD LOGIC - Simple and foolproof
  useEffect(() => {
    if (!isReload) return;
    ...
    // Handle different reload scenarios
    setTimeout(() => {
      // 1. If there's a popup showing, resume it
  ...
  setCurrentSourceElement('lotus-1');
  setShowPopupBook(true);
  // CRITICAL: Resume the symbol discovery sequence properly
  setTimeout(() => {
    sceneActions.updateState({
      discoveredSymbols: {
        ...sceneState.discoveredSymbols,
        lotus: true  // Ensure symbol is marked as discovered
      }
    });
    
    // CONTINUE THE FLOW: Trigger the celebration after discovery
    setTimeout(() => {
  ...
  setTimeout(() => {
    sceneActions.updateState({
      discoveredSymbols: {
        ...sceneState.discoveredSymbols,
        trunk: true  // Ensure symbol is marked as discovered
      }
    });
  ...
  break;
          ...
          case 'final_fireworks':
            ...
            // ENSURE COMPLETION STATE IS SET ON RELOAD
            sceneActions.updateState({ 
              gameCoachState: null,  
              isReloadingGameCoach: false,
            ...
            return; // Exit early for fireworks
        }
        
        // Clear reload flag for popups
          sceneActions.updateState({ isReloadingGameCoach: false });

      }
      
      // 2. If GameCoach was showing, resume it with proper flow
        ...
        setTimeout(() => {
          switch(sceneState.gameCoachState) {
           case 'lotus_wisdom':
  console.log('🌸 Showing lotus wisdom on reload');
  showMessage("Wonderful! Like the lotus, you're growing wiser! Ganesha would be proud!", {
    duration: 8000,
    animation: 'bounce',
    position: 'top-right'
  });
  ...
  // CRITICAL: Mark as shown IMMEDIATELY to prevent double trigger
  sceneActions.updateState({ 
    gameCoachState: null,
    lotusWisdomShown: true,
    readyForWisdom: false  // Clear the trigger condition
  });
  break;
              
           case 'elephant_wisdom':
  console.log('🐘 Showing elephant wisdom on reload');
  showMessage("Amazing! You've learned about Ganesha's strength and kindness!", {
    duration: 8000,
    animation: 'bounce',
    position: 'top-right'
  });
  ...
  break;
            ...
            case 'mastery_wisdom':
              console.log('🏆 Showing mastery wisdom on reload');
              showMessage("You've become a keeper of Ganesha's wisdom! Ready for your next adventure?", {
                duration: 8000,
                animation: 'bounce',
                position: 'top-right'
              });
              // For mastery, show scene completion after message
              setTimeout(() => {
                sceneActions.updateState({ 
                  gameCoachState: null,
                  isReloadingGameCoach: false 
      ...
      // 3. No special reload needed
      else {
        console.log('🔄 No special reload needed, clearing flags');
// Clear reload flag for popups after delay to ensure symbol discovery completes
setTimeout(() => {
  sceneActions.updateState({ isReloadingGameCoach: false });
}, 1500);      }
  ...
  setCurrentSourceElement('lotus-1');  // ← CHANGE TO lotus-1 OR null
              setShowPopupBook(true);
              setShowSparkle(null);
            }, 1000);
            return;
  ...
  const showSymbolCelebration = (symbol) => {
    ...
    switch(symbol) {
      case 'lotus':
        ...
        sceneActions.updateState({ currentPopup: 'lotus_card' });
      ...
      case 'trunk':
        ...
        sceneActions.updateState({ currentPopup: 'trunk_card' });
        setCardContent({ title, image, stars });
        setShowMagicalCard(true);
        break;
      ...
      case 'final':
        ...
        sceneActions.updateState({ 
          ...
          phase: PHASES.COMPLETE,  // SET PHASE TO COMPLETE
          stars: 5,               // SET STARS FOR GAMEWELCOMESCREEN
          completed: true,        // SET COMPLETED FOR GAMEWELCOMESCREEN
        ...
        safeSetTimeout(() => {
          ...
          // COMPLETE THE SCENE PROPERLY FOR GAMEWELCOMESCREEN
  ...
  const handleCloseCard = () => {
    ...
    if (cardContent.title?.includes("Lotus Symbol")) {
      ...
      setTimeout(() => {
        sceneActions.updateState({ 
          readyForWisdom: true,
          gameCoachState: 'lotus_wisdom'  // SET STATE FOR RELOAD TRACKING
        });
        setPendingAction('show-golden-lotus');
      }, 1500);
    }
      ...
      setTimeout(() => {
        sceneActions.updateState({ 
          readyForWisdom: true,
          gameCoachState: 'elephant_wisdom'  // SET STATE FOR RELOAD TRACKING
        });
        setPendingAction('bloom-golden-lotus');
      }, 500);
    }
      ...
      sceneActions.updateState({
        phase: PHASES.COMPLETE,
        stars: 5,               // FOR GAMEWELCOMESCREEN
        completed: true,        // FOR GAMEWELCOMESCREEN
        progress: {
          percentage: 100,
          starsEarned: 5,
          completed: true
      ...
      // COMPLETE SCENE FOR GAMEWELCOMESCREEN DETECTION
      sceneActions.completeScene(5, {
        symbolsFound: ['lotus', 'trunk', 'golden'],
        completed: true,
        totalStars: 5
