...
//import ganeshaCoach from "./assets/images/characters/ganesha-character.png";
...
// Enhanced showMessage that tracks coach visibility
const showCoachMessage = (message, options = {}) => {
  setIsCoachVisible(true);
  
  // If hint system is showing, hide it
  if (progressiveHintRef.current) {
    progressiveHintRef.current.hideHint();
  }
  
  // Call the showMessage from useGameCoach hook, not this function
  const { showMessage: gameCoachShowMessage } = useGameCoach();
  gameCoachShowMessage(message, {
    ...options,
    onClose: () => {
...
function handleGoldenLotusClick() {
  ...
  showCoachMessage("The golden lotus is special in ancient traditions!", {
    duration: 3000
  });
...
// Add ref for progressive hint system
const progressiveHintRef = useRef(null);

  // Hint system state
  const [hintsEnabled, setHintsEnabled] = useState(true);
  
  // Function to hide active hints
const hideActiveHints = () => {
  if (progressiveHintRef.current) {
    progressiveHintRef.current.hideHint();
    progressiveHintRef.current.resetHintLevel();
  }
};
  ...
  const messages = [
    {
      id: 'all-lotus-message',
      condition: () => (sceneState?.phase === PHASES.ALL_BLOOMED || 
                      sceneState?.lotusStates?.every(state => state === 1)) && 
                      !sceneState?.messagesShown?.['all-lotus-message'],
      content: (
        <div className="scroll-image-container">
          <img src={popup1} alt="All lotus bloomed!" className="scroll-image" />
        </div>
      ),
      type: 'scroll'
    },
    {
      ...
      condition: () => isReload && 
                      ...
                      !sceneState?.messagesShown?.['reload-hint'],
      content: <p>Click the golden lotus to continue your journey!</p>,
      type: 'popup'
    }
  ];
  ...
  const createWaterDrop = () => {
    ...
    const trunkPositionRight = 20; // Match with trunk position in CSS
    const trunkPositionBottom = 30; // Match with trunk position in CSS
  ...
  // Add handler for hint level changes
const handleHintLevelChange = (level) => {
  console.log(`Hint level changed to: ${level}`);
...
showMessage("The golden lotus is special in ancient traditions!", {
  ...
  showCoachMessage("Great job! You found all the lotus flowers!", {
    duration: 3000
  });
} else {
  ...
  if (bloomedCount === 1) {
    showCoachMessage("Wonderful! You found your first lotus flower!", {
      duration: 3000
    });
  }
};
  ...
  const handleElephantClick = () => {
    ...
    if (sceneState.elephantTransformed) {
      console.log('Elephant already transformed');
      return;
    }
    ...
    // Use GameCoach to explain what's happening
showCoachMessage("The elephant's trunk represents Ganesha, who removes obstacles!", {
  duration: 4000,
  priority: 5
});
    
    // Transform the elephant
    safeSetTimeout(() => {
      sceneActions.updateState({ 
...
showCoachMessage("You've mastered the pond scene! On to the next adventure!", {
  duration: 5000,
  priority: 10
});
      }
...
const getProgressiveHintConfigs = () => [
  // Lotus hints
  {
    id: 'lotus-hint',
    message: 'Try clicking on the lotus flowers!',
    explicitMessage: 'Click the pink lotus flower to make it bloom.',
    position: { top: '40%', left: '30%' },
  ...
  // Golden lotus hint
  {
    id: 'golden-lotus-hint',
    message: 'The golden lotus looks special!',
    explicitMessage: 'Click on the golden lotus in the center!',
    position: { top: '45%', left: '45%' },
    condition: (sceneState, hintLevel) => {
      if (!sceneState) return false;
      return sceneState.goldenLotusVisible && 
  ...
  {
    id: 'elephant-hint',
    message: 'What will the elephant do?',
    explicitMessage: 'Click on the elephant to see magic happen!',
    position: { bottom: '45%', right: '25%' },
    condition: (sceneState, hintLevel) => {
      if (!sceneState) return false;
      return sceneState.elephantVisible && 
...
/>
            ...
            {sceneState.elephantVisible && (
              <div className={`elephant-partial ${sceneState.elephantTransformed ? 'elephant-position-locked' : ''}`}>
                <ClickableElement
                  id="elephant"
...
<ProgressiveHintSystem
  ...
  sceneId={sceneId}
  sceneState={sceneState}
  hintConfigs={getProgressiveHintConfigs()}
