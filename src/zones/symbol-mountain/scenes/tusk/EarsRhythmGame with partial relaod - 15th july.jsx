...
const EarsRhythmGame = ({ 
  ...
  console.log('🎵 EarsRhythmGame inline render:', { isActive, currentNote });
  ...
  }, [isActive, currentNote, discoveredInstruments]); // ← Added discoveredInstruments dependency
...
// ✅ IMPROVED: Fix timing issue for reload
useEffect(() => {
  if (isReload && isActive && currentSequence.length > 0) {
    console.log('🔄 EARS RHYTHM RELOAD: Restoring game state', {
      initialGamePhase,
      initialPlayerInput,
      currentNote,
      currentSequence
    ...
    if (initialGamePhase && initialGamePhase !== 'waiting') {
      ...
      if (initialGamePhase === 'listening' && initialPlayerInput.length < currentSequence.length) {
        ...
        setSequenceItemsShown(currentSequence.length); // ← ADD: Show full sequence
        console.log('🔄 RELOAD: Player can continue clicking');
      }
    }
    
    console.log('✅ EARS RHYTHM RELOAD: State restored');
  }
}, [isReload, isActive, initialGamePhase, initialPlayerInput, currentSequence]);

// ✅ ADD: Separate useEffect to handle reload when sequence is ready
useEffect(() => {
  if (isReload && isActive && currentSequence.length > 0 && initialGamePhase === 'listening') {
    ...
    const restoreTimer = setTimeout(() => {
      console.log('🔄 DELAYED RELOAD: Restoring listening state');
  ...
  const handleInstrumentClick = async (instrumentType) => {
    ...
    if (newPlayerInput[currentIndex] === currentSequence[currentIndex]) {
      ...
      // ✅ ADD THIS: Save to parent scene state for reload
