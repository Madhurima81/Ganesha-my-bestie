...
// SimpleGameCoach handles all messaging internally - no hook needed
const hideCoach = () => {}; // Keep for navigation cleanup
const clearManualCloseTracking = () => {}; // Keep for navigation cleanup
  ...
  useEffect(() => {
    console.log('🧹 ASSEMBLY V2: Cleaning GameCoach on scene entry');
...
// Trigger final celebration after mastery message
useEffect(() => {
  // Only trigger if mastery was just shown and we haven't started celebration yet
  if (sceneState?.masteryShown && 
      !sceneState?.currentPopup && 
      !showSceneCompletion && 
      Object.keys(sceneState?.placedSymbols || {}).length === 8) {
    
    console.log('🎆 Triggering final celebration after mastery message');
    
    safeSetTimeout(() => {
      showFinalCelebration();
    }, 3000); // 3 seconds after mastery message
  }
}, [sceneState?.masteryShown, sceneState?.currentPopup, showSceneCompletion]);
...
if (zone) {
  ...
  // Complete assembly
  console.log('🎆 8th symbol placed - triggering completion sequence');
  ...
  safeSetTimeout(() => {
    ...
    // DIRECT celebration trigger (independent of GameCoach)
    console.log('🎆 Starting final celebration immediately');
    showFinalCelebration();
      ...
      >
        <div className="sacred-assembly-container">
          {/* Sacred Mountain Background */}
           {/* NEW: SimpleGameCoach replaces all old message logic */}
{/* SimpleGameCoach DISABLED for testing */}
{false && (
  <SimpleGameCoach
    config={SimpleGameCoachConfigs.sacredAssembly(sceneState?.placedSymbols)}
    sceneState={sceneState}
    sceneActions={sceneActions}
    profileName={profileName}
    onSparkleEffect={(type) => {
      console.log(`✨ SimpleGameCoach triggered sparkle: ${type}`);
      setShowSparkle(type);
    }}
  />
)}

{/* MANUAL FLOW TESTING - Watch console logs */}
{console.log('🎮 MANUAL FLOW DEBUG:', {
  placedSymbols: Object.keys(sceneState?.placedSymbols || {}).length,
  phase: sceneState?.phase,
  completed: sceneState?.completed,
