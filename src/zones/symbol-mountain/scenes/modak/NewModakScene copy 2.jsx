...
const getCollectedModakPosition = (displayIndex) => {
  ...
  // Add this after line 190 (after getCollectedModakPosition function)

// ✅ UTILITY: Bulletproof save function (reusable across all scenes)
const saveCompletion = (sceneId, completionData) => {
  console.log(`🔒 BULLETPROOF SAVE for ${sceneId}:`, completionData);
  
  let saveSuccess = false;
  
  // METHOD 1: Standard onComplete (preferred)
  try {
    if (onComplete) {
      onComplete(sceneId, completionData);
      console.log('✅ Method 1: onComplete succeeded');
      saveSuccess = true;
    }
  } catch (error) {
  ...
  // METHOD 2: Direct GameStateManager save (bulletproof backup)
  try {
    ...
    if (activeProfileId) {
      ...
      if (progressData.zones && progressData.zones[zoneId] && progressData.zones[zoneId].scenes) {
        ...
        console.log('✅ Method 2: Direct save succeeded', {
          totalStars,
          completedScenes,
          sceneData: progressData.zones[zoneId].scenes[sceneId]
        });
        saveSuccess = true;
      }
    }
  ...
  // METHOD 3: Scene state backup (for recovery)
  try {
    ...
    console.log('✅ Method 3: Scene state backup saved');
  } catch (error) {
    console.error('❌ Method 3 failed:', error);
  }
  
  if (!saveSuccess) {
    console.error('🚨 ALL SAVE METHODS FAILED! Manual recovery needed');
  }
  
  return saveSuccess;
};

// ✅ GUARD: Auto-save when scene reaches complete phase (insurance policy)
useEffect(() => {
  if (sceneState?.phase === PHASES.COMPLETE && 
      !sceneState?.autoSaved && 
      sceneState?.completed) {
    
    console.log('🛡️ COMPLETION GUARD: Auto-saving completed scene');
    
    const completionData = {
      stars: sceneState.stars || 8,
      symbols: sceneState.discoveredSymbols || { mooshika: true, modak: true, belly: true },
      completed: true,
      totalStars: sceneState.stars || 8
    };
    
    saveCompletion(sceneId, completionData);
    
    // Mark as auto-saved to prevent duplicates
    sceneActions.updateState({ autoSaved: true });
  }
}, [sceneState?.phase, sceneState?.completed, sceneState?.autoSaved]);
...
sceneActions.updateState({
  ...
  const showSymbolCelebration = (symbol) => {
    ...
    switch(symbol) {
      ...
      case 'final':
  console.log('🔒 FINAL COMPLETION: Starting bulletproof save sequence');
  
  // ✅ IMMEDIATE BULLETPROOF SAVE (before any UI changes)
  const completionData = {
    stars: 8,
  ...
  const saveResult = saveCompletion('modak', completionData);
  
  if (saveResult) {
    console.log('✅ Completion saved successfully, proceeding with celebration');
  } else {
    console.error('❌ Save failed, but continuing with celebration');
  }
  ...
  safeSetTimeout(() => {
    ...
    // Show completion screen (no onComplete needed - already saved)
...
{/* Add this debug button in NewModakScene render */}
<button 
  style={{
    position: 'fixed', 
    top: '300px', 
  ...
  onClick={() => {
    ...
    if (onComplete) {
      ...
      console.log('✅ Called onComplete manually');
    } else {
      console.error('❌ onComplete function not found!');
    }
  }}
>
  🧪 Test onComplete
...
{/* 🚑 RECOVERY BUTTON (remove in production) */}
<button 
  style={{
    position: 'fixed', 
    top: '360px', 
  ...
  onClick={() => {
    console.log('🚑 RECOVERY: Manually forcing completion save');
    const completionData = {
      stars: 8,
      symbols: { mooshika: true, modak: true, belly: true },
      completed: true,
    ...
    saveCompletion('modak', completionData);
  }}
>
  🚑 Force Save
</button>
...
{showSparkle === 'final-fireworks' && (
  <Fireworks
    ...
    onComplete={() => {
      console.log('Fireworks complete - NOT setting scene completion here');
      setShowSparkle(null);
      // ✅ REMOVED: Don't call setShowSceneCompletion here
      // The safeSetTimeout above already handles it
    }}
  />
)}
...
<SceneCompletionCelebration
  ...
  // For Replay:
  onReplay={() => {
  console.log('🔧 REPLAY: Completion already saved, just navigating');
  onNavigate?.('zone-welcome');
}}
onContinue={() => {
  console.log('🔧 CONTINUE: Completion already saved, just navigating');
  onNavigate?.('zone-welcome');
}}
/>
