  ...
  const handleMoundClick = (moundIndex) => {
    ...
    if (moundIndex === sceneState.correctMound) {
      ...
      safeSetTimeout(() => {
        ...
        safeSetTimeout(() => {
          sceneActions.updateState({ 
            moundsVanished: true,  // Add this new state
...
const collectedCount = newCollectedModaks.length;  // ✅ Use the actual collection array    
  ...
  // 🔧 COMPLETE FIX: Replace your handleRockFeed function with this
...
/*const handleRockFeed = (dragData) => {
  ...
  console.log(`Rock fed ${newFeedCount} times via drag & drop`);
...
const handleRockFeed = (dragData) => {
  ...
  if (newFeedCount >= 5) {
  // Rock fully fed - FIRST transform rock to belly
  safeSetTimeout(() => {
    console.log('🪨➡️🌌 Transforming rock to belly first');
    setShowSparkle('belly-transform');
    
    sceneActions.updateState({ 
      rockTransformed: true,  // Transform rock to belly FIRST
      phase: PHASES.ROCK_TRANSFORMED
    });
    
    // THEN start belly discovery after transformation
    safeSetTimeout(() => {
      sceneActions.updateState({ 
        symbolDiscoveryState: 'belly_discovering',
        currentPopup: 'belly_info'
      ...
      setShowPopupBook(true);
      setShowSparkle(null);
    }, 2000); // Wait for transformation to complete
