...
const DELETION_BOUNDARY = {
  left: 5,    // 5% from left edge
  right: 95,  // 95% from left edge  
  top: 5,     // 5% from top
  bottom: 95  // 95% from top
};

// ADD THIS - Check if position is outside mandap area
const isOutsideMandapArea = (position) => {
  const left = parseFloat(position.left);
  const top = parseFloat(position.top);
  
  return (
    left < DELETION_BOUNDARY.left || 
    left > DELETION_BOUNDARY.right ||
    top < DELETION_BOUNDARY.top || 
    top > DELETION_BOUNDARY.bottom
  );
};
...
const [gameState, setGameState] = useState(() => {
  ...
  const [showDeletionWarning, setShowDeletionWarning] = useState(null);
...
const updateDecorationPosition = (decorationKey, newPosition) => {
  // Just update position normally - don't delete during dragging
  setGameState(prev => {
  ...
  // Check if approaching deletion boundary (but don't delete yet)
  const nearBoundary = (
    parseFloat(newPosition.left) < 15 || 
    parseFloat(newPosition.left) > 85 ||
    parseFloat(newPosition.top) < 15 || 
    parseFloat(newPosition.top) > 85
  );
  
  // Show warning only once per drag session
  if (nearBoundary && !warningShown.has(decorationKey)) {
    setWarningShown(prev => new Set(prev).add(decorationKey));
    setGaneshaMessage("Careful! Drag too far and it will disappear!");
    setShowGaneshaMessage(true);
    safeSetTimeout(() => setShowGaneshaMessage(false), 1500);
  }
};

const handleDragEnd = (decorationKey) => {
  setIsDragging(false);
  
  // Clear warning state for this decoration
  setWarningShown(prev => {
    const newSet = new Set(prev);
    newSet.delete(decorationKey);
    return newSet;
  });
  
  // Check final position for deletion
  const currentPosition = gameState.decorationPositions?.get(decorationKey);
  if (currentPosition && isOutsideMandapArea(currentPosition)) {
    // DELETE the decoration
    setGameState(prev => {
      const newPlacements = removeDecorationFromZone(decorationKey, prev.placedDecorations);
      const newPositions = new Map(prev.decorationPositions);
      newPositions.delete(decorationKey);
      
      const newCount = Math.max(0, prev.decorationCount - 1);
      const newStars = Math.max(0, Math.min(8, Math.floor(newCount * 1.2)));
    ...
    playDeletionSound();
    setGaneshaMessage("Decoration removed! You can always add more!");
    setShowGaneshaMessage(true);
    safeSetTimeout(() => setShowGaneshaMessage(false), 2000);
  }
};
...
// REPLACE YOUR EXISTING removeDecorationFromZone function with this:
const removeDecorationFromZone = (decorationKey, placedDecorations) => {
  const newPlacements = new Map(placedDecorations);
  
  // Parse the decoration key: "decoration-{zoneId}-{index}"
  const keyParts = decorationKey.split('-');
  const targetZoneId = keyParts[1];
  const decorationIndex = parseInt(keyParts[2]);
  
  if (newPlacements.has(targetZoneId)) {
    const existing = newPlacements.get(targetZoneId);
    const decorationsArray = Array.isArray(existing) ? existing : [existing];
    
    if (decorationsArray.length === 1) {
      // Remove the entire zone entry if only one decoration
      newPlacements.delete(targetZoneId);
    } else {
      // Remove specific decoration from array
      const updatedDecorations = decorationsArray.filter((_, index) => index !== decorationIndex);
      newPlacements.set(targetZoneId, updatedDecorations);
    }
  }
...
// ADD THIS DELETION HANDLER FUNCTION
const handleDecorationDeletion = (decorationKey) => {
  setGameState(prev => {
    const newPlacements = removeDecorationFromZone(decorationKey, prev.placedDecorations);
    const newPositions = new Map(prev.decorationPositions);
    newPositions.delete(decorationKey);
  ...
  return (
    <div className="mandap-decoration-container">
...
{Array.from(gameState.placedDecorations.entries()).map(([zoneId, decorations]) => {
  ...
  return decorationsArray.map((decoration, index) => {
    ...
    return (
// ENHANCED FreeDraggableItem with warning system
<FreeDraggableItem
  key={decorationKey}
  id={decorationKey}
  ...
  onPositionChange={(newPosition) => {
    // Check if approaching deletion boundary
    const nearBoundary = (
      parseFloat(newPosition.left) < 15 || 
      parseFloat(newPosition.left) > 85 ||
      parseFloat(newPosition.top) < 15 || 
    ...
    if (nearBoundary && !showDeletionWarning) {
      setShowDeletionWarning(decorationKey);
      setGaneshaMessage("Careful! Drag too far and it will disappear!");
      setShowGaneshaMessage(true);
      safeSetTimeout(() => {
        setShowGaneshaMessage(false);
        setShowDeletionWarning(null);
      }, 1500);
    }
    
    updateDecorationPosition(decorationKey, newPosition);
  }}
  onDragStart={() => setIsDragging(true)}
onDragEnd={() => handleDragEnd(decorationKey)}
>
  <img 
    src={DECORATION_IMAGES[decoration.image]}
    alt={decoration.name}
className={`decoration-image ${decoration.hasLightEffect ? 'glowing' : ''} ${
  warningShown.has(decorationKey) ? 'deletion-warning' : ''
}`}
    style={{ 
